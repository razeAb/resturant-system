...
export default function WorkerDashboard() {
  ...
  return (
    <div>
      ...
      {activeShift ? <button onClick={stopShift}>Stop Shift</button> : <button onClick={startShift}>Start Shift</button>}

      <h3>Monthly Hours</h3>
      <ul>
        {Object.entries(monthly).map(([key, hrs]) => (
          <li key={key}>
            {key}: {hrs.toFixed(2)} hrs
          </li>
        ))}
      </ul>
      ...
      <ul>
        {shifts.map((s) => (
          <li key={s._id}>
            ...
            {s.adjustedByManager && s.adjustedAt && <em> (Manager adjusted on {new Date(s.adjustedAt).toLocaleDateString()})</em>}
          </li>
        ))}
      </ul>
    </div>
