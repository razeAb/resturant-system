...
export default function WorkingHoursPage() {
  ...
  const filtered = useMemo(
    () => (range.from || range.to ? shifts.filter((s) => inRange(s.start)) : shifts),
    [shifts, range]
  );
  ...
  const toTime = (v) =>
    v
      ? new Date(v).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit", hour12: false })
      : "--:--";
      ...
      <header className="max-w-7xl mx-auto flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white rounded-full px-3 py-2 shadow-sm border border-gray-200">
            ...
            <button className="text-gray-500" title="הגדרות">⚙️</button>
          </div>
        </div>
      </header>
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s._id} className="border-b last:border-b-0 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4">{s.shiftName || "כללי"}{/* General */}</td>
